...
def plan_full_vehicle_routes(dow: int, target_date: datetime):
    ...
    # ========================================
    # FİNAL BOŞALTMA - Rota bitiminde kasada yük varsa tesise git
    # ========================================
    print(f"\n🏭 FİNAL BOŞALTMA KONTROLÜ...")
    for v in vehicles_data:
        if v["load"] > 0:
            # Kasada yük var, Yenikent tesisine git
            unload_dist = haversine_km_vectorized(
                v["pos"][1], v["pos"][0],
                unload_pos[1], unload_pos[0]
            ) * 1.3
            ...
            print(f"   ✓ Araç {v['id']}: {v['load']:.2f} ton → Yenikent tesisine boşaltma")
            v["load"] = 0.0
